...
let compare = (function($) {
    ...
    let tpl = {
        filters: _.template($('#compare-filters-template').html() || '',null, replaceFilter),
        header: _.template($('#compare-header-template').html() || '',null, replaceFilter),
        props: _.template($('#compare-props-template').html() || '',null, replaceFilter)
    };
    ...
    function updateCompareTab() {
        let comparedGoodsStr = $.cookie(settings.cookie.goods),
            comparedGoodsArr = comparedGoodsStr ? comparedGoodsStr.split(',') : [],
            comparedCategoryId = $.cookie(settings.cookie.category),
            compareHref = '/compare' + (comparedGoodsArr.length ? '#' + encodeURIComponent(comparedCategoryId + '|' + comparedGoodsStr) : '');
    ...
    function _getBaseProps(goods) {
        ...
        let valuesWithIds, values, equal;
    ...
    function _initComparePage() {
        let hashData = decodeURIComponent(location.hash).substr(1).split('|'),
