...
let order = (function($) {
    ...
    function _changeDelivery() {
        let $item = ui.$delivery.btn.filter(':checked'),// - выбранная радиокнопка со способом доставки
            deliveryType = $item.attr('data-type'), //- способ
            deliverySumma = freeDelivery.enabled ? 0 : +$item.attr('data-summa'),//- логично, сумма.
            cartSumma = cart.getSumma(),// - стоимость всех товаров из корзины
    ...
    function _onSubmitForm(e) {
        let isValid,    //валидна ли форма
            formData,
            cartData,
            orderData;
        e.preventDefault(); //чтобы при сабмите наша страница перезагружалась
        ui.$alertValidation.addClass('hidden'); //скрываем блок с ошибкой валидации
        isValid = _validate();
        if (!isValid) {
            ui.$alertValidation.removeClass('hidden');
            return false;
        }
        formData = ui.$orderForm.serialize();//начинаем собирать данные
        cartData = _getCartData(); //Данные для корзины, напоминаю, хранятся у нас в localStorage и доступны через cart
